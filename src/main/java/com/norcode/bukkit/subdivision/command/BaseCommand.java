...
public class BaseCommand implements TabExecutor {
	...
	public BaseCommand(SubdivisionPlugin plugin, String name, String[] aliases, String requiredPermission, String[] help) {
		this.plugin = plugin;
		this.name = name;
	...
	public final boolean onCommand(CommandSender commandSender, org.bukkit.command.Command command, String label, String[] args) {
		try {
			...
			Chat.sendMessage(commandSender, error.getMessage());
		}
		return true;
	}
	...
	protected void showHelp(CommandSender sender, String label, LinkedList<String> args) {
		if (help.length > 0) {
			Chat.sendMessage(sender, help);
		}
		if (subcommands.size() > 0) {
			Chat.sendMessage(sender, "Sub-Commands: " + StringUtils.join(filterByPermission(sender, subcommands).keySet(), ", "));
		}
	}
